...
impl BrowserStorage {
    ...
    pub async fn store_memory(&mut self, memory: &SignedMemory) -> Result<(), String> {
        ...
        let success = Reflect::get(&result, &"success".into()).unwrap().as_bool().unwrap_or(false);
        if !success {
    ...
    async fn call_sql_execute(&self, sql: &str, params: &Array) -> Result<Object, String> {
        ...
        let sql_function: js_sys::Function = sql_execute.dyn_into()
        ...
        let result = sql_function.apply(&JsValue::NULL, &args)
        ...
        result.dyn_into().map_err(|_| "Invalid result format".to_string())
    }
    
    async fn call_sql_query(&self, sql: &str, params: &Array) -> Result<Object, String> {
        let window = web_sys::window().ok_or("No window")?;
        let sql_query = js_sys::Reflect::get(&window, &"sqlQuery".into())
            .map_err(|_| "sqlQuery not found")?;
        let sql_function: js_sys::Function = sql_query.dyn_into()
