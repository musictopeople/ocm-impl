...
fn handle_connection(mut stream: TcpStream) {
    ...
    let (status_line, content_type, contents) = if request_line.starts_with("GET / ") {
        ...
        ("HTTP/1.1 404 NOT FOUND".to_string(), "text/html".to_string(), "404 Not Found".as_bytes().to_vec())
    };
...
fn serve_file(file_path: &str, content_type: &str) -> (String, String, Vec<u8>) {
    if Path::new(file_path).exists() {
        match fs::read(file_path) {
            Ok(contents) => ("HTTP/1.1 200 OK".to_string(), content_type.to_string(), contents),
            Err(_) => ("HTTP/1.1 500 INTERNAL SERVER ERROR".to_string(), "text/html".to_string(), "Internal Server Error".as_bytes().to_vec()),
        }
    } else {
        ("HTTP/1.1 404 NOT FOUND".to_string(), "text/html".to_string(), "File not found".as_bytes().to_vec())
    }
}